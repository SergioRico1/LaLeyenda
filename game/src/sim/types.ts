 ...
 *
 * 3 — PRODUCTION.md §1's captain. A version-2 save has an island, timers and a
 * week of progress and NO identity; the migration gives it one rolled from its
 * own seed rather than sending it back to creation (see MIGRATIONS[2]).
 */
...
/* --------------------------------------------------------------------------
 * the captain — PRODUCTION.md §1
 * ----------------------------------------------------------------------- */

/** The seven model families under `public/assets/models/av_*`. */
export type AvatarSlot = 'body' | 'hair' | 'beard' | 'eyes' | 'hat' | 'top' | 'bottom';

/**
 * What the captain is wearing, as MODEL IDS AND NOTHING ELSE.
 *
 * The sim never learns what a GLB is: a part is the string `av_hat_captain`,
 * and render/assets.ts is the only thing that knows it becomes
 * `assets/models/av_hat_captain.glb`. src/sim/captain.ts owns the table of
 * which strings are legal per slot.
 *
 * Three slots are nullable because a captain can genuinely not have them —
 * clean-shaven, bare-headed, both eyes. The other four are not: a look with no
 * body is not a look.
 */
...
export interface Captain {
  /** What goes on a leaderboard row. Sanitized on the way in (captain.ts). */
  name: string;
  /**
   * The captain's own island.
   *
   * PRODUCTION.md §2: "Every player currently gets the same seeded island. The
   * seed should be the captain's, rolled or chosen at creation." It is stored
   * here as well as on `GameState.seed` because it is the captain's property —
   * the two are set together at creation and `setCaptain` refuses to let them
   * drift, since the island has already been generated by then.
   */
  seed: string;
  look: CaptainLook;
}
...
export interface GameState {
  ...
  /** Who is playing. Never null after createNewGame; a save written before the
   *  captain existed is given one by MIGRATIONS[2] rather than being sent back
   *  to creation with an island already on it. */
  captain: Captain;
